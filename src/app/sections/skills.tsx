...
function Skills({ isEnUs }: SectionProps) {
  ...
  return (
    <>
      {selectedSkill && (
        <Modal
          handleOverlayClick={() => setSelectedSkill(null)}
          skill={selectedSkill}
        ></Modal>
      )}
      <div
...
const Modal = ({ skill, handleOverlayClick }: ModalProps) => {
  ...
  return (
    <div
      ...
      className="fixed z-50 inset-0 bg-black bg-opacity-10 flex justify-center items-center"
    >
      <div id={hrefs.SKILLS} className="fixed top-[30%] w-[65%] right-center">
        <div
          data-aos="fade-up"
        ...
        >
          ...
          <h1>{isEnUs ? skill.descriptionUs : skill.description}</h1>
        </div>
      </div>
    </div>
  );
